...
export default function DockCTA() {
    const [showDock, setShowDock] = useState(false);

    const items: DockItemData[] = [
        {
            icon: <RocketIcon />,
            label: "KI-Agents anfragen",
            onClick: () => { window.location.hash = '#kontakt'; },
        },
        {
            icon: <WorkflowIcon />,
            label: "Automatisierung anfragen",
            onClick: () => { window.location.hash = '#kontakt'; },
        },
        {
            icon: <ContactIcon />,
    ...
    return (
        ...
        >
            ...
            <button
                ...
                className="relative border-0 rounded-[calc(0.75rem-0.2rem)] px-5 py-2 items-center font-medium cursor-pointer text-sm bg-sapphire text-white flex gap-2"
            >
                <span style={{ fontFamily: 'var(--font-syne)' }}>Kontakt</span>
            ...
            {/* Dock Popup */}
            <div
                className={`absolute top-[calc(100%+12px)] right-0 z-50 transition-all duration-300 ${showDock ? 'opacity-100 translate-y-0 pointer-events-auto scale-100' : 'opacity-0 translate-y-2 pointer-events-none scale-95'}`}
            >
                <Dock
                    items={items}
                    panelHeight={56}
